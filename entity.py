...
def remove_html_tags(text):
    clean = re.compile('<.*?>')
    text_cleaned = re.sub(clean, '', text)
...
class KGEntity(object):
    ...
    def __init__(self, enity_id, label, description, types, source):
        ...
        self.ident = enity_id
        self.label = remove_html_tags(str(label))
        self.desc = remove_html_tags(str(description)) #sometimes provides a very concrete type or additional semantics
        self.types = types  #set of semantic types
        self.source = source  #KG of origin: dbpedia, wikidata or google kg
        
    
    def __repr__(self):
        return "id: %s, label: %s, description: %s" % (self.ident, self.label, self.desc)
